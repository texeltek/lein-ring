...
* `:webxml` -
  web.xml file to use in place of auto-generated version (relative to project
  root).

The following command will start a development web server that runs the war
file generated by `lein ring uberwar`. This is useful for testing war-specific
options.

    lein ring server-uberwar
